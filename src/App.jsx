...
// Generate mock data around a center point
const generateMockData = (centerLat, centerLng) => {
  const types = ['Medical Emergency', 'Trapped / Rescue Needed', 'Food / Water Required', 'Safe'];
  const sources = ['verified', 'crowd', 'crowd', 'ngo'];
  const data = [];
  for (let i = 0; i < 100; i++) {
    ...
    if (type === 'Safe') source = 'verified'; // User marked safe is verified
    
    const lat = centerLat + latOffset;
    const lng = centerLng + lngOffset;
    
    let description = type === 'Safe' ? 'User marked safe' : 'Automated distress signal detected.';
    if (source === 'ngo') {
    ...
    data.push({
      id: 1000 + i,
      type: source === 'ngo' ? 'NGO Center' : type,
...
function App() {
  ...
  useEffect(() => {
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        setHeatmapZones([
          { lat: latitude + 0.02, lng: longitude + 0.03, radius: 1500, type: 'High Flood Risk' },
          { lat: latitude - 0.03, lng: longitude - 0.02, radius: 2000, type: 'Structural Collapse Warning' }
        ]);
      },
      async (error) => {
        ...
        setHeatmapZones([
          { lat: 51.52, lng: -0.06, radius: 1500, type: 'High Flood Risk' },
          { lat: 51.48, lng: -0.12, radius: 2000, type: 'Structural Collapse Warning' }
        ]);
      }
    );
  }, []);
  ...
  return (
    <div style={{ position: 'relative', width: '100vw', height: '100vh', overflow: 'hidden' }}>
      ...
      <RightSidebar inventory={inventory} leaderboard={leaderboard} />
      ...
      {!isCommsOpen && (
        ...
        >
          <MessageSquare size={20} /> OPEN COMMS
        </button>
      )}
